...
LOG_PATH = os.path.join(os.path.dirname(__file__), "simple_AI_agent.log")
...
def _log(entry: dict) -> None:
    ...
    Each line is a JSON object containing a timestamp and a free‑form ``type`` field
    describing the kind of message (prompt, response, tool‑call, etc.). The API key is
    masked for security.
    """
...
def _log_gemini_interaction(model_name: str, user_input: str, response_text: str) -> None:
    """Log the Gemini request/response payload (excluding the API key)."""
    _log({
...
def _call_gemini(user_input: str) -> str:
    ...
    The function logs the interaction payload for debugging/audit purposes.
    """
    last_error = None
    for name in [MODEL_NAME, FALLBACK_MODEL_NAME]:
        ...
        try:
            ...
            _log_gemini_interaction(name, user_input, response_text)
